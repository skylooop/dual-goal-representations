...
class GCIVLVisualVIBAgent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
    ...
    def value_loss(self, batch, grad_params, rng):
        ...
        goal_reps, kl_loss, kl_info = self.network.select('vib')(grad_goals, rng, encoded=False, params=grad_params)

        (next_v1_t, next_v2_t) = self.network.select('target_value')(next_obs, jax.lax.stop_gradient(goal_reps))
        next_v_t = jnp.minimum(next_v1_t, next_v2_t)
        q = batch['rewards'] + self.config['discount'] * batch['masks'] * next_v_t

        (v1_t, v2_t) = self.network.select('target_value')(obs, jax.lax.stop_gradient(goal_reps))
        v_t = (v1_t + v2_t) / 2
        adv = q - v_t

        q1 = batch['rewards'] + self.config['discount'] * batch['masks'] * next_v1_t
        q2 = batch['rewards'] + self.config['discount'] * batch['masks'] * next_v2_t
        (v1, v2) = self.network.select('value')(grad_obs, goal_reps, params=grad_params)
        v = (v1 + v2) / 2
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        goal_reps, _, _ = self.network.select('vib')(goals, rng, encoded=False)

        v1, v2 = self.network.select('value')(obs, jax.lax.stop_gradient(goal_reps))
        nv1, nv2 = self.network.select('value')(next_obs, jax.lax.stop_gradient(goal_reps))
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
        adv = nv - v
        ...
        grad_goal_reps, _, _ = self.network.select('vib')(goals, rng, encoded=False, params=grad_params)
    ...
    def create(
        ...
        network_info = dict(
            ...
            vib=(vib_def, (ex_encoder_outputs, jax.random.PRNGKey(0), False)),
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gcivl_vib_vis',  # Agent name.

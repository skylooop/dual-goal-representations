...
class CRLTRAAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng=None):
        ...
        # Actor gradients do not improve performance, as seen in the case of other base algorithms.
        _, _, goal_reps = self.network.select("contrastive")(
        ...
        if self.config["actor_loss"] == "awr":
            ...
            q1, q2 = self.network.select("critic")(
                batch["observations"], goal_reps, batch["actions"]
            )
            q = jnp.minimum(q1, q2)
            adv = q - v
            ...
            q1, q2 = self.network.select("critic")(
                batch["observations"], goal_reps, q_actions
            )
            q = jnp.minimum(q1, q2)
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        contrastive_loss, contrastive_info = self.contrastive_rep_loss(
            batch, grad_params
        )
        ...
        loss = critic_loss + value_loss + actor_loss + contrastive_loss
        return loss, info

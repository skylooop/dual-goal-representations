...
class GCIVLVisualVIBAgent(flax.struct.PyTreeNode):
    ...
    def value_loss(self, batch, grad_params, rng):
        ...
        goal_reps, kl_loss, kl_info = self.network.select("vib")(
            grad_goals, rng, encoded=False, params=grad_params
        )

        (next_v1_t, next_v2_t) = self.network.select("target_value")(
            next_obs, jax.lax.stop_gradient(goal_reps)
        )
        next_v_t = jnp.minimum(next_v1_t, next_v2_t)
        q = batch["rewards"] + self.config["discount"] * batch["masks"] * next_v_t

        (v1_t, v2_t) = self.network.select("target_value")(
            obs, jax.lax.stop_gradient(goal_reps)
        )
        v_t = (v1_t + v2_t) / 2
        adv = q - v_t
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        grad_goal_reps, _, _ = self.network.select("vib")(
            goals, rng, encoded=False, params=grad_params
        )

...
class GCIVLVisualVIPAgent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
    ...
    def rep_loss(self, batch, grad_params):
        ...
        grad_final_obs = self.network.select("goal_encoder")(
            batch["rep_final_obs"], params=grad_params
        )
        grad_init_obs = self.network.select("state_encoder")(
            batch["rep_init_obs"], params=grad_params
        )
        grad_k_obs = self.network.select("state_encoder")(
            batch["rep_k_obs"], params=grad_params
        )
        grad_next_k_obs = self.network.select("state_encoder")(
            batch["rep_k+1_obs"], params=grad_params
        )
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name="gcivl_vip_vis",  # Agent name.

...
class GCIQLAgent(flax.struct.PyTreeNode):
    ...
    def _dayan_advantage(self, batch):
        ...
        def scalar_value(s, g):
            return value_fn(s[None], g[None])[0]
    ...
    def actor_loss(self, batch, grad_params, rng=None):
        ...
        When actor_loss='awr', the advantage mode is controlled by
        config['actor_advantage_mode']:
        - 'qv':     Q(s, a, g) - V(s, g)  (default, standard IQL)
        - 'vdelta': V(s', g) - V(s, g)  (V-only, no Q needed)
        - 'td':     r + γ·mask·V(s', g) - V(s, g)  (standard TD advantage)
        - 'dayan':  r + (s' - s)·∇_s V(s, g)  (Dayan Eq. 4, gradient-based)
        """
        if self.config["actor_loss"] == "awr":
            # AWR loss with configurable advantage mode.
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            actor_advantage_mode="qv",  # AWR advantage: 'qv' (Q-V), 'vdelta' (V(s')-V(s)), 'td' (r+γV(s')-V(s)), 'dayan' (r+(s'-s)·∇V).

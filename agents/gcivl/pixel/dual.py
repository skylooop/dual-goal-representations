...
class GCIVLVisualDualAgent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
    ...
    def rep_loss(self, batch, grad_params):
        ...
        q1, q2 = self.network.select('rep_critic')(grad_obs, grad_goals, batch['actions'], params=grad_params)
    ...
    def value_loss(self, batch, grad_params):
        ...
        grad_goal_reps = self.network.select('rep_value')(
            grad_goals, params=grad_params if self.config['value_grad'] else None
        )
    ...
    def actor_loss(self, batch, grad_params, rng=None):
        ...
        grad_goal_reps = self.network.select('rep_value')(
            grad_goals, params=grad_params if self.config['actor_grad'] else None
        )
    ...
    def create(
        ...
        network_info = dict(
            ...
            rep_value=(rep_value_def, (ex_encoder_outputs, ex_encoder_outputs)),
            rep_critic=(rep_critic_def, (ex_encoder_outputs, ex_encoder_outputs, ex_actions)),
            target_rep_critic=(copy.deepcopy(rep_critic_def), (ex_encoder_outputs, ex_encoder_outputs, ex_actions)),
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gcivl_dual_vis',  # Agent name.
            ...
            rep_expectile=0.7,  # IVL expectile for learning the representation value function.
            goalrep_dim=64,  # Dimensionality of the temporal goal representation.
            rep_type='mrn',  # Parameterization of the temporal goal representation (see utils/temporal.py).
            value_grad=False,  # Whether to flow gradients from the value loss to the representation.
            actor_grad=False,  # Whether to flow gradients from the actor loss to the representation.

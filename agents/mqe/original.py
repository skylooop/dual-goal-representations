...
class MQEAgent(flax.struct.PyTreeNode):
    ...
    def mrn_distance(self, x: jnp.ndarray, y: jnp.ndarray):
        ...
        def mrn_distance_component(x: jnp.ndarray, y: jnp.ndarray):
            ...
            l2_component: jnp.ndarray = jnp.linalg.norm(
                (x - y) * (1 - mask) + eps, axis=-1
            )
        ...
        dists: jnp.ndarray = jax.vmap(
            mrn_distance_component, in_axes=(-1, -1), out_axes=-1
        )(x_split, y_split)
    ...
    def critic_loss(self, batch, grad_params, critic_rng):
        ...
        use_next_state = jax.random.bernoulli(
            critic_rng, p=self.config["next_state_sample"], shape=(batch_size,)
        )
        ...
        psi_next = self.network.select("psi")(
            intermediate_value_goals, params=grad_params
        )
        ...
        def compute_backup(dist, dist_next):
            ...
            one_step_mask = jnp.where(
                use_next_state_mask.reshape(
                    use_next_state_mask.shape[0],
                ),
                1.0,
            ...
            optim_value = (
                1 - jax.lax.stop_gradient(dist_next) + jnp.log(gamma) * one_step_mask
            )
            optim_value = (
                optim_value * (1 - dw)
                + jnp.diagonal(optim_value, axis1=1, axis2=2)[..., None] * dw
            )
        ...
        backup_loss, optim_backup = compute_backup(
            dist, jax.lax.stop_gradient(dist_next)

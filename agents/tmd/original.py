...
class TMDAgent(flax.struct.PyTreeNode):
    ...
    def total_loss(self, batch, grad_params, rng=None, contrastive_only=False):
        ...
        (
            ...
            critic_loss,
            critic_info,
        ) = self.critic_loss(batch, grad_params)
    ...
    def update(self, batch, contrastive_only=False):
        ...
        def loss_fn(grad_params):
            return self.total_loss(
                batch, grad_params, rng=rng, contrastive_only=contrastive_only
            )
    ...
    def sample_actions(
        ...
        if self.config["use_latent"]:
            psi_s, psi_g = (
                self.network.select("psi")(observations),
                self.network.select("psi")(goals),
            )
